...
int Hash(char* key, ulong_t seed)
{
    ...
    return h % SYMBOL_SIZE;
}

void PrintSymbols(symbol_t* symbol)
{
    if (symbol)
    {
        ...
        if (symbol->next)
        {
            printf("collision happened at slot %d with key: %s and key: %s \n", Hash(symbol->key, RANDOM_SEED), symbol->key, symbol->next->key);
